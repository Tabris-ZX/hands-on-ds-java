...
public class Time implements Comparable<Time> {
    ...
    public Time(String str) {
        // 1. 检查空值和长度（"HH:MM MM-DD" 长度为11）
        if (str == null || str.length() != 11) {
            throw new IllegalArgumentException("时间格式必须是 HH:MM MM-DD（长度为11），当前输入: " + str);
        }
        
        // 2. 检查分隔符位置
        if (str.charAt(2) != ':') {
            throw new IllegalArgumentException("时间格式必须是 HH:MM MM-DD，第3位必须是 ':'，当前输入: " + str);
        }
        if (str.charAt(5) != ' ') {
            throw new IllegalArgumentException("时间格式必须是 HH:MM MM-DD，第6位必须是空格，当前输入: " + str);
        }
