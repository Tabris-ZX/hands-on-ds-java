...
public class TripManager {
     ...
     *   <li>arrival_time: 到达时间（格式 HH:MM MM-DD）</li>
     * </ul>
     * 
    ...
    public void addTrip(long userID, TripInfo trip) throws SQLException {
        PreparedStatement ps = conn.prepareStatement(
                "INSERT INTO trip_info (user_id, train_id, departure_station, arrival_station, type, duration, price, departure_time, arrival_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
        );
        ps.setLong(1, userID);
        ...
        ps.setString(8, trip.getDepartureTime().toString());
        ps.setString(9, trip.getArrivalTime().toString());
        ps.executeUpdate();
        ps.close();
    }
    ...
    public List<TripInfo> queryTrip(long userID) throws SQLException {
        ...
        while (rs.next()) {
            ...
            t.setDepartureTime(new boyuai.trainsys.util.Time(rs.getString("departure_time")));
            t.setArrivalTime(new boyuai.trainsys.util.Time(rs.getString("arrival_time")));
            trips.add(t);
        }
        rs.close();
        ps.close();
    ...
    public void removeTrip(long userID, TripInfo trip) throws SQLException {
        PreparedStatement ps = conn.prepareStatement(
                "DELETE FROM trip_info WHERE user_id=? AND train_id=? AND departure_station=? AND arrival_station=? AND type=? AND departure_time=?"
        );
        ps.setLong(1, userID);
        ps.setString(2, trip.getTrainID().toString());
        ps.setInt(3, trip.getDepartureStation().value());
        ps.setInt(4, trip.getArrivalStation().value());
        ps.setInt(5, trip.getType());
        ps.setString(6, trip.getDepartureTime().toString());
        ps.executeUpdate();
        ps.close();
    }
}
