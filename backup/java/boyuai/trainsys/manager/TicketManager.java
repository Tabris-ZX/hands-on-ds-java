...
public class TicketManager {
    ...
    public TicketManager(String filename) {
        this.ticketInfo = new BPlusTree<>(filename, FixedString.class, TicketInfo.class);
    }
    ...
    public int querySeat(FixedString trainID, Date date, int stationID) {
        ...
        for (int i = 0; i < relatedInfo.length(); i++) {
            ...
            if (info.getDate().equals(date) && info.getDepartureStation() == stationID) {
    ...
    public int updateSeat(FixedString trainID, Date date, int stationID, int delta) {
        ...
        for (int i = 0; i < relatedInfo.length(); i++) {
            ...
            if (info.getDate().equals(date) && info.getDepartureStation() == stationID) {
                // 创建更新后的票务信息
                TicketInfo updatedInfo = new TicketInfo(info);
                updatedInfo.setSeatNum(info.getSeatNum() + delta);

                // 更新B+树中的信息
                ticketInfo.modify(trainID, info, updatedInfo);
     ...
     * @return 车票信息列表
     */
    public SeqList<TicketInfo> queryTickets(FixedString trainID) {
        return ticketInfo.find(trainID);
    }
     ...
     * @return 车票信息列表
     */
    public SeqList<TicketInfo> queryTicketsByDate(FixedString trainID, Date date) {
        SeqList<TicketInfo> allTickets = ticketInfo.find(trainID);
        SeqList<TicketInfo> result = new SeqList<>();
        ...
        for (int i = 0; i < allTickets.length(); i++) {
            TicketInfo info = allTickets.visit(i);
            if (info.getDate().equals(date)) {
                result.insert(result.length(), info);
            }
        }
     ...
     * @param date 日期
     * @param stationID 始发站ID
     * @param ticketNum 需要的票数
     * @return 是否有足够余票
     */
    public boolean hasEnoughTickets(FixedString trainID, Date date, int stationID, int ticketNum) {
        int available = querySeat(trainID, date, stationID);
        return available >= ticketNum && available != -1;
    }
